...
void DemoBaseApplLayer::handleLowerMsg(cMessage* msg)
{
    ...
    if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
        ...
        var = simTime()-msg->getCreationTime();
        appoggio = appoggio + var;

        avgTimeApp = appoggio/receivedWSMs; //variabile utile per calcolare media, non potendolo fare nella function finish()
...
void DemoBaseApplLayer::handleSelfMsg(cMessage* msg)   //non entra qui
{
    switch (msg->getKind()) {
...
void DemoBaseApplLayer::finish()
{
    ...
    recordScalar("avgTimeApplicativo", avgTimeApp);
}
...
void DemoBaseApplLayer::startService(Channel channel, int serviceId, std::string serviceDescription)   //qua non entriamo
{
